...
휴대폰·태블릿 등 터치 기기로 접속하면 화면에 **터치 컨트롤(방향패드 + 액션 버튼)** 이 자동으로 표시됩니다.
- 왼쪽 방향패드: ◀ ▶ 이동, ▲ ▼ 는 메뉴 이동용
- 오른쪽 버튼: 점프 / 활공 / 할퀴 / 불(파이어볼). 메뉴 화면에서는 **점프 버튼이 "선택"** 역할을 합니다.
- 가로 모드(landscape)로 두면 화면이 더 크게 보입니다.
- PC에서 터치 컨트롤을 미리 보고 싶으면 주소 뒤에 `?touch=1` 을 붙이면 됩니다.
